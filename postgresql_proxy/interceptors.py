...
class Interceptor:
    ...
    def _get_plugin_interceptor_function(self, interceptor):
        if plugin := self.plugins.get(interceptor.plugin):
            if func := getattr(plugin, interceptor.function, None):
                ...
                raise Exception(
                    "Can't find function {} in plugin {}".format(
                        interceptor.function, interceptor.plugin
                    )
                )
        else:
    ...
    def get_codec(self):
        if self.context is not None and "connect_params" in self.context:
            if (
                self.context["connect_params"] is not None
                and "client_encoding" in self.context["connect_params"]
            ):
                return self.convert_encoding_to_python(
                    self.context["connect_params"]["client_encoding"]
                )
        return "utf-8"
...
class CommandInterceptor(Interceptor):
    ...
    def _intercept_context_data(self, data):
        ...
        if b"client_encoding" in entries:
            self.context["connect_params"]["client_encoding"] = entries[
                b"client_encoding"
            ].decode("ascii")
